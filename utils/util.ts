...
export function getTagColor(CONSTANT: ConstantItem[], color: string): string {
...
//   return urlLink;
// };
export const extractIdAndSlug = (
    str: string,
): { id: number; alias: string } | null => {
    if (!str) return null;

    // bỏ .html nếu có
    const clean = str.replace(/\.html$/, "").toLowerCase();
